...
* **Resizing an already-admitted `PodGang`.** A Kueue `Workload`'s PodSets are fixed at creation. Grove scaling operations that would change the Pod count of an existing `PodGang` (as opposed to creating a new `PodGang`, such as an additional `PodCliqueScalingGroup` replica) have no way to update an already-admitted Workload's PodSet counts in phase 1.
